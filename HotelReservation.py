...
df_ = pd.read_excel("datasets/HotelEkzel.xlsx")
df = df_.copy()
df.head()
...
# dt is another data frame
def hotel_data_prep2(dataframe,dt):
    ...
    dataframe["discount"] = dataframe["avg_price_per_room"] / dataframe["avg_price_by_month"]
    ...
    cat_cols, num_cols, cat_but_car = grab_col_names(dt, cat_th=7, car_th=20)
    ...
    ohe_cols = [col for col in cat_cols if ((7 >= dt[col].nunique() > 2) and
    ...
    binary_cols = [col for col in cat_cols if
                   dt[col].dtype not in [int, float] and (dt[col].nunique() == 2) and (
                               col != "BOOKING_STATUS")]


    for col in binary_cols:
        label_encoder(dt, col)


    #num_cols = [col for col in dt.columns if
     #           dt[col].dtype in ['int64', 'float64'] and col != 'BOOKING_STATUS']

   # dataframe = one_hot_encoder(dt, ohe_cols)
    return dataframe
"""""
data prep 2
"""""
...
joblib.dump(final_model1, 'lgbm_model.pkl')
